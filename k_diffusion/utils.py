...
# 1) ── save a vertical grid of all C channels (C = 7 for FoJ) ───────────
def save_foj_channel_grid(foj_batch,      # Tensor [N, 7, H, W]  (on GPU or CPU)
                          filename,
                          normalize=True,
                          nrow=None,
                          padding=2):
    """
    Creates 7 separate make_grid() outputs (one per channel) and stacks
    them top-to-bottom into a single PIL image.

    Arguments
    ---------
    foj_batch : torch.Tensor
        Shape [N, 7, H, W]   (values typically in [−1,1] or unbounded)
    filename  : str / Path
        Where to save the resulting PNG.
    """
    foj_batch = foj_batch.detach().cpu()

    N, C, H, W = foj_batch.shape
    assert C == 7, f"Expected 7 channels, got {C}"

    # default grid width  ≈ sqrt(N)
    nrow = nrow or math.ceil(N ** 0.5)
    row_grids = []
    for c in range(C):
        ch = foj_batch[:, c:c+1]                       # keep as single-channel
        grid = vutils.make_grid(ch,
                                nrow=nrow,
                                padding=padding,
                                normalize=normalize)
        row_grids.append(grid)

    big_grid = torch.cat(row_grids, dim=1)             # stack vertically
    vutils.save_image(big_grid, filename)
    print(f"[foj_vis] wrote {filename}")


# 2) ── overlay junctions and θ on conditioning image ────────────────────
def overlay_batch(rgb_batch, foj_batch, extractor_fn,
                  save_dir, vis_fn,
                  **vis_kwargs):
    """
    Loops over a mini-batch, converts each item to PIL, extracts
    junctions/params via `extractor_fn`, overlays via your
    visualize_junctions_and_theta(), then saves one PNG per sample.

    Arguments
    ---------
    rgb_batch     : FloatTensor [N, 3, H, W]  in range [0,1]
    foj_batch     : FloatTensor [N, 7, H, W]
    extractor_fn  : callable(rgb, foj) -> (junctions, params_dict)
        • junctions    : iterable[(x,y)]
        • params_dict  : {(x,y): {'labels': .., 'theta': .., 'sign': ..}}
    save_dir      : str / Path
    vis_fn        : callable(image, junctions, params_dict, **kwargs)
        your visualize_junctions_and_theta() function
    vis_kwargs    : forwarded to `vis_fn` (num_rays, vis_radius, …)
    """
    ...
    for i in range(N):
        ...
        # 2-a) tensor → PIL for the background image
        pil_img = vutils.to_pil_image(rgb.clamp(0,1))

        # 2-b) user-supplied logic to parse FoJ and return junctions/params
        junctions, params = extractor_fn(rgb, foj)

        # 2-c) draw overlay
        overlay = vis_fn(pil_img, junctions, params, **vis_kwargs)

        fname = os.path.join(save_dir, f"sample_{i:03}.png")
        overlay.save(fname)
        print(f"[foj_vis] wrote {fname}")