...
# ── 3. Run training ──────────────────────────────────────────────────
python train.py \
  --config configs/foj_transformer_v2_onetwo_obj_udf_more_data.json \
  --batch-size 32 \
  --checkpointing \
  --start-method fork \
  --num-workers 4 \
  --name foj_diffusion_onetwo_obj_udf_moredata \
  --evaluate-every 10000 \
  --demo-every 100000 \
  --end-step 10000000 \
  --wandb-project foj_diffusion_onetwo_obj_udf_moredata \
