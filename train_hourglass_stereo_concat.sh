...
# ── 3. Run training ──────────────────────────────────────────────────
python train.py \
  --config /n/netscratch/zickler_lab/Lab/linbo/stereo_project/k-diffusion/configs/foj_transformer_v2_stereo.json \
  --batch-size 32 \
  --checkpointing \
  --start-method fork \
  --num-workers 4 \
  --name stereo_concat_2ch \
  --evaluate-every 50000 \
  --demo-every 100000000 \
  --end-step 10000000 \
  --wandb-project stereo_diffusion_concat_2ch \
